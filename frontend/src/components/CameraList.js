...
const CameraList = () => {
  ...
  useEffect(() => {
     depart = departmentId || user?.departmentId;
    if (!depart) {
      console.warn("No department ID found.");
      return; // Exit if no departmentId is found
    }
    ...
    async function fetchCameras() {
      try {
        ...
        const response = await fetch(
          `${process.env.REACT_APP_API_URL}/api/camera/getCamerasByDepartmentId/${depart}`,
          {
            method: "GET",
            headers: {
          ...
          >
            ...
            {user.role === "Super Admin" && (
              <div className="flex justify-around p-4 bg-transparent">
                <div
                  onClick={() => handleUpdate(camera._id)}
                ...
                <div
                  ...
                  className="cursor-pointer flex flex-col items-center text-red-500"
                >
                  <FaTrash size={20} />
                  <span className="text-sm">Delete</span>
                </div>
                <div
                  onClick={() => handleInfo(camera._id)}
                ...
                <div
                  onClick={() => handleInfo(camera._id)}
                  className="cursor-pointer flex flex-col items-center text-green-500"
                >
                  <FaToggleOn size={20} />
