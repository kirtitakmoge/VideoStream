    ...
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 m">
        <div className="col-span-1 shadow-md">
          ...
          <Link to={`/surgeonList/${departmentId}`} className="button bg-gray-200 p-4 rounded-md block hover:bg-gray-300 w-full">
            <h2 className="text-2xl text-center font-bold mb-4">Surgeons</h2>
          </Link>
        </div>
        <div className="col-span-1 shadow-md">
          {/* Style the link to resemble a button */}
          <Link to={`/cameralist/${departmentId}`} className="button bg-gray-200 p-4 rounded-md block hover:bg-gray-300 w-full">
            <h2 className="text-2xl text-center font-bold mb-4">Live stream from camera</h2>
          </Link>
        </div>
        <div className="col-span-1  shadow-md ">
          <Link to={`/deviceListadmin/${departmentId}`} className="bg-gray-200 p-4 rounded-md block hover:bg-gray-300">
            <h2 className="text-2xl font-bold mb-4">Recorded Video from camera</h2>
            
          </Link>
        </div>
        <div className="col-span-1 shadow-md">
          <Link to={`/createCamera/${departmentId}/${department_name}`} className="bg-gray-200 block p-4 rounded-md hover:bg-gray-200">
            <h2 className="text-2xl text-center font-bold mb-4">Add new Camera</h2>
          </Link>
        </div>
        <div className="col-span-1 shadow-md">
          <Link to={`/patientData/${departmentId}`} className="bg-gray-200 block p-4 rounded-md hover:bg-gray-300">
            <h2 className="text-2xl text-center font-bold mb-4">Patients</h2>
          </Link>
        </div>
        <div className="col-span-1 shadow-md">
          {/* Button to show delete modal */}
          <button onClick={() => setShowDeleteModal(true)} className="button bg-gray-200 p-4 rounded-md block hover:bg-gray-300 w-full">
            <h2 className="text-1xl  text-center font-bold mb-4">Remove Department</h2>
          </button>
        </div>
        <div className="col-span-1 shadow-md">
          {/* Button to show rename modal */}
          <button onClick={() => setShowRenameModal(true)} className="button bg-gray-200 p-4 rounded-md block hover:bg-gray-300 w-full">
            <h2 className="text-1xl font-bold mb-4">Rename Department</h2>
          </button>
        </div>
      </div>
